...
class Animes(commands.Cog, name="animes"):
    ...
    async def search_anime(cache: dict, **kwargs) -> None:
        if cache['status'] == 'created':
            ...
            embed = anime_embed(cache['anime_search'].data[cache['index']], cache['color'])
            embed.set_footer(text=f"Anime {cache['index'] + 1} de {len(cache['anime_search'].data)}")
            ...
            if kwargs['emoji'] == '📝':
                if AnimesList.get(user_id=kwargs['user'].id, mal_id = cache['anime_search'].data[cache['index']].mal_id, dubbed=cache['dubbed']):
                    ...
                    anime_title = anime_title if not cache["dubbed"] else f"{anime_title} (Dublado)"

                    await cache['message'].channel.send(f'`{anime_title}` adicionado a lista!')
        ...
        if cache['status'] == 'created':
            ...
            for i in cache['anime_list']:
                ...
                if len(cache['animes']):
                    ...
                    embed.set_footer(text=f"Animes: {cache['index'] + 1}/{len(cache['animes'])} - Pesquisando: {len(cache['animes'])}/{len(cache['anime_list'])}")
                    await message.edit(embed=embed, content=None)
                
                else:
                    await message.edit(embed=None, content="A lista de animes está vazia.")
            ...
            if len(cache['animes']):
                ...
                await cache['message'].edit(embed=None, content="A lista de animes está vazia.")
    ...
    async def search_anime(self, ctx: commands.Context, *, anime: str) -> None:
        ...
        if animes.data:
            scr = Script(name=f'{ctx.author.id}_search_anime', function_name='search_anime', time_out=300)
            await scr.execute(ctx=ctx, anime_search=animes, color=self.bot.color, dubbed=dubbed)
    ...
    async def anime_list(self, ctx: commands.Context, user: Optional[DUser] = None) -> None:
        if not user:
            user = ctx.author
        
        animes = AnimesList.get_user(user_id=user.id)
        if animes:
            scr = Script(name=f'{ctx.author.id}_anime_list', function_name='anime_list', time_out=300)
            await scr.execute(ctx=ctx, animes=animes, color=self.bot.color, jikan=self.jikan)
