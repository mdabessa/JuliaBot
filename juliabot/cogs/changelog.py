...
class ChangelogCog(commands.Cog):
    ...
    async def on_ready(self):
        ...
        for server in servers:
            ...
            date = updates[0].date
            date_str = (
                f"em {datetime.strptime(date, '%Y-%m-%d').strftime('%d/%m/%Y')}"
                if date
                else "recentemente"
            )
    ...
    async def changelog_channel(self, ctx: commands.Context):
        ...
        if server.changelog_channel:
            ...
            if channel:
                ...
                await ctx.send(
                    "O canal de changelog automático definido não existe mais. Use `!remove_changelog_channel` para limpar a configuração."
                )
        else:
            await ctx.send(
                "Nenhum canal de changelog automático definido. Use `!set_changelog_channel` para definir um."
            )
