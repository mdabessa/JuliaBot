...
from ..rl_analyzer import replay_analyzer, RANKS
...
class RLAnalyzer(commands.Cog, name="rlanalyzer"):
    ...
    def embed_player(player: dict, color) -> Embed:
        ...
        embed.add_field(name="Rank:", value=RANKS[player["tier"]-1].capitalize().replace('-', ' '), inline=False)
        embed.add_field(name="Analyzer Rank:", value=RANKS[player["predict"]["tier"]-1].capitalize().replace('-', ' '), inline=False)
        embed.add_field(name="Score:", value=player["stats"]["core"]["score"], inline=True)
        embed.add_field(name="Goals:", value=player["stats"]["core"]["goals"], inline=True)
        embed.add_field(name="Saves:", value=player["stats"]["core"]["saves"], inline=True)
    ...
    async def _replay_analyzer(cache: dict, **kwargs):
        if cache['status'] == 'created':
            ...
            file = File(f'./juliabot/images/rl_logos/{player["predict"]["tier"]}.png', filename=f'tier_{player["predict"]["tier"]}.png')
    ...
    async def replay_analyzer(self, ctx: commands.Context, replay_id: str):
        ...
        if (user is None) or (user.ballchasing_token is None):
            await ctx.send('Você precisa configurar um token de acesso com o comando `rl_token`. O token pode ser obtido em `ballchasing.com` gerando uma API Key.')
            return
        ...
        if response.status_code == 401:
            await ctx.send('O seu token está errado. Você precisa configurar um token de acesso com o comando `rl_token`. O token pode ser obtido em `ballchasing.com` gerando uma API Key.')
            return
